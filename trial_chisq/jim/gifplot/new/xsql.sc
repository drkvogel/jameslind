...
{	CLOCK	eclock;
	...
	EXEC SQL INQUIRE_SQL ( :error_msg = ERRORTEXT );
...
int xsql_init_main( char *db_name, char *access_options )
{
	...
	EXEC SQL WHENEVER SQLERROR CALL xsql_error_sql;
	do
		{xsql_error_status = 0;
		if ( EOS == acc_opt[0] )
			{
			...
			{
			EXEC SQL CONNECT :database OPTIONS = :acc_opt;
			}
		}
...
int xsql_exproc( char *cmd, int *return_value )
{
	...
	do
		{xsql_error_status = 0;
		EXEC SQL EXECUTE PROCEDURE :proc INTO :ret_val;
		}
		while ( xsql_error_status > 0 && xsql_error_retry >= 0 );
...
{	int	i;
	...
	da_ptr = &(d->idesc);
	EXEC SQL PREPARE :dyn_word INTO da_ptr FROM :sel_cmd;
...
void xsql_dyn_cursor_init( XSQL_DYN *xd, char *access_mode )
{
	...
	EXEC SQL DECLARE :cursor_name CURSOR FOR :query_name;
	do
		{xsql_error_status = 0;
...
int xsql_query_fetch( XSQL_DYN *xd )
{
	...
	cursor_name = xd->cur_id;
	desc = &(xd->idesc);
	EXEC SQL FETCH :cursor_name USING DESCRIPTOR :desc;
...
int xsql_dyn_execute( XSQL_DYN *xd, char *cmd )
{
	...
	EXEC SQL PREPARE oneoff FROM :comman;
	EXEC SQL EXECUTE oneoff USING DESCRIPTOR :query_name;
