...
ccc        IF (PGBEG(0,'?',1,1) .NE. 1) STOP
cc	  IF (PGBEG(0,'C:\FORTRAN\GRAPHS\test.GIF/GIF',1,1) .NE. 1) STOP
c      PAUSE 1

c	  IF (PGBEG(0,'/tmp/test.gif/GIF',1,1) .NE. 1) STOP
c      IF (PGBEG(0,'/user/cp/public_html/cjb/p.gif/GIF',1,1) .NE. 1) STOP
      IF (PGBEG(0,'p.gif/GIF',1,1) .NE. 1) STOP
...
c	  or=real(dt*(nt-dt))/real(dc*(nc-dc))
	...
	print*,' '
	print*,' logodds=',logodds,' logci1=',logci1,' logci2=',logci2
	print*,' p=',p,' p2=',p2
