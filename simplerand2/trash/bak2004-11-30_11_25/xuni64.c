...
/* return number of bytes, excluding null terminator, of base64 string output, or -1 if the conversion failed */
int multibyte_to_uni64(unsigned const char* szMBCSIn, unsigned char** szUCB64Out)
{
    wchar_t* wszTemp = NULL;
    int result = 0;

    result = multibyte_to_wide(szMBCSIn, &wszTemp);
...
/* return number of characters converted, or -1 if an invalid multibyte character is encountered */
...
int wide_to_uni64(const wchar_t* pwszWideIn, unsigned char** szUCB64Out)
{
    ...
    if (0 != xb64_encode(pUCS2, cchWideChar * 2, szUCB64Out, &cchUCB64Out)) {
        result = cchUCB64Out;
    } else {
        result = -1;
    }
...
/* return number of wide chars written, excluding null terminator, or -1 if the number of UCS-2 bytes is uneven */
int uni64_to_wide(unsigned char* szUCB64In, wchar_t** lpWideOut)
{
    ...
    result = xb64_decode(szUCB64In, strlen(szUCB64In), &rgbUCS2Out, &cbUCS2Out);
...
int wide_to_multibyte(const wchar_t* lpWideIn, unsigned char** szMBCSOut)
{
    ...
    cbMultiByte = wcstombs(*szMBCSOut, lpWideIn, 0);
