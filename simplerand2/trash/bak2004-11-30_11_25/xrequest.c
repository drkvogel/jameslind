...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
static	int		req_abcmenu_show = YES;		/* SHOW ABC-MENU */
...
{       int     yn, ynlow;
	int     code = -1;
	long    c;
	if ( YES != req_abcmenu_show )				/* OMIT MENU */
		{aside( "CONTINUE_OR_QUIT option intercepted" );
		return( REQ_STATE_ABCMENU );
		}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
		/* DETERMINE WHETHER OR NOT ABC-MENU IS TO BE SHOWN */
void request_abcmenu_show( int show )
{	req_abcmenu_show = ( 0 == show ) ? NO : YES;
}
/*---------------------------------------------------------------------------*/
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
					/* GET AUXILLARY YES-NO ANSWER */
void request_verify_txt( int *v_status, int *v_answer, char *txt )
{
	request_vshow_aux_tmp( txt );
	request_verify( v_status, v_answer );
	request_vshow_aux_tmp( "" );
}
/*---------------------------------------------------------------------------*/
...
{       int     verify_answer;
	...
	while ( 1 )
		...
		if ( NULL != low_abs && date_cmp( answer, low_abs ) < 0 )
			{date_to_string( dbuf, low_abs, 0 );  /* STRONG CHECK */
			request_vshow_aux_tmp( ( NULL == la_msg )
...
: "question requires a 24-hour time entered as hour:min:sec, e.g. 14:52:30" ) );
	...
	while ( 1 )
		...
		if ( NULL != low_abs && clock_cmp( answer, low_abs ) < 0 )
			{clock_to_string( cbuf, low_abs, 0 ); /* STRONG CHECK */
			request_vshow_aux_tmp( ( NULL == la_msg )
		...
		else if ( NULL != low_opt && clock_cmp( answer, low_opt ) < 0 )
			{clock_to_string( cbuf, low_opt, 0 );  /* WEAK CHECK */
			request_vshow_aux_tmp( ( NULL == lo_msg )
		...
		else if ( NULL != upp_opt && clock_cmp( answer, upp_opt ) > 0 )
...
{       int     i, ncols, kl, tl, max_len;
	...
	request_show_exp( ( NULL != explain_msg ) ? explain_msg
		: "Select one choice (enter code) from the menu listed below" );
...
/*===========================================================================*/
		/* ASK RANDOMIZATION QUESTION ABOUT XPACK-FORMAT COUNTRIES */
void request_xpack_country( int nq, int *status, char *question,
	char *explain_msg, char *country_code, char *country_name )
{
	int	i, indx;
	long	nc;
	COUNTRY	*c = NULL;
	REQUEST_MENU	*r = NULL;
	xpack_country_select_active( &nc, &c );
	r = request_menu_init( "countries" );
	if ( NULL == r )
		{bug ( "Cannot create country menu");
		exit( 0 );
		}
	for ( i = 0; i < nc; i++ )		/* SHOW ACTIVE COUNTRIES */
		{request_menu_add_item( r, c[i].code, c[i].name );
		}
	if ( NULL != c )
		{xfree( c );
		}		
	request_menu( nq, status, question, &indx, explain_msg, r );
	if ( REQUEST_OK( *status ) )
		{request_menu_copy_key( country_code, r, indx );
		if ( NULL != country_name )
			{request_menu_copy_txt( country_name, r, indx );
			}
		}
	request_menu_free( r );
}
/*---------------------------------------------------------------------------*/
		/* ASK RANDOMIZATION QUESTION ABOUT XPACK-FORMAT CENTRES */
void request_xpack_centre( char *country_code,
	int nq, int *status, char *question,
	char *explain_msg, char *centre_code, char *centre_name )
{
	int	i, indx;
	long	ncen;
	CENTRE	*cen = NULL;
	REQUEST_MENU	*r = NULL;
	xpack_centre_select_active( &ncen, &cen, country_code );
	r = request_menu_init( "centres" );
	if ( NULL == r )
		{bug ( "Cannot create centre menu");
		exit( 0 );
		}
	for ( i = 0; i < ncen; i++ )		/* SHOW ACTIVE CENTRES */
		{request_menu_add_item( r, cen[i].code, cen[i].name );
		}
	if ( NULL != cen )
		{xfree( cen );
		}
	request_menu( nq, status, question, &indx, explain_msg, r );
	if ( REQUEST_OK( *status ) )
		{request_menu_copy_key( centre_code, r, indx );
		if ( NULL != centre_name )
			{request_menu_copy_txt( centre_name, r, indx );
			}
		}
	request_menu_free( r );
}
/*---------------------------------------------------------------------------*/
		/* ASK RANDOMIZATION QUESTION ABOUT XPACK-FORMAT STAFF */
void request_xpack_staff( char *centre_code,
	int nq, int *status, char *question,
	char *explain_msg, char *staff_code, char *staff_name )
{
	int	i, indx;
	long	nstf;
	STAFF	*stf = NULL;
	REQUEST_MENU	*r = NULL;
	xpack_staff_select_active( &nstf, &stf, centre_code );
	r = request_menu_init( "staff" );
	if ( NULL == r )
		{bug ( "Cannot create staff menu" );
		exit( 0 );
		}
	for ( i = 0; i < nstf; i++ )		/* SHOW ACTIVE STAFF */
		{request_menu_add_item( r, stf[i].code, stf[i].name );
		}
	if ( NULL != stf )
		{xfree( stf );
		}
	request_menu( nq, status, question, &indx, explain_msg, r );
	if ( REQUEST_OK( *status ) )
		{request_menu_copy_key( staff_code, r, indx );
		if ( NULL != staff_name )
			{request_menu_copy_txt( staff_name, r, indx );
			}
		}
	request_menu_free( r );
}
